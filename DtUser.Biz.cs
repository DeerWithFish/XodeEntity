...
public partial class DtUser : Entity<DtUser>
{
    #region 对象操作
    static DtUser()
    {
    ...
    //    if (XTrace.Debug) XTrace.WriteLine("开始初始化DtUser[用户]数据……");

    //    var entity = new DtUser();
    //    entity.Name = "abc";
    //    entity.Password = "abc";
    ...
    //    if (XTrace.Debug) XTrace.WriteLine("完成初始化DtUser[用户]数据！");
    //}
    ...
    public static DtUser FindByID(Int32 id)
    {
        if (id <= 0) return null;
    ...
    /// <returns>实体对象</returns>
    public static DtUser FindByName(String name)
    {
        ...
        // 单对象缓存
        //return Meta.SingleCache.GetItemWithSlaveKey(name) as DtUser;
    ...
    public static IList<DtUser> FindAllByCode(String code)
    {
        if (code.IsNullOrEmpty()) return new List<DtUser>();
    ...
    /// <returns>实体列表</returns>
    public static IList<DtUser> FindAllByMail(String mail)
    {
        if (mail.IsNullOrEmpty()) return new List<DtUser>();
    ...
    /// <returns>实体列表</returns>
    public static IList<DtUser> FindAllByMobile(String mobile)
    {
        if (mobile.IsNullOrEmpty()) return new List<DtUser>();
    ...
    /// <returns>实体列表</returns>
    public static IList<DtUser> FindAllByRoleID(Int32 roleId)
    {
        if (roleId <= 0) return new List<DtUser>();
    ...
    /// <returns>实体列表</returns>
    public static IList<DtUser> Search(String name, String mail, String mobile, String code, Int32 roleId, DateTime start, DateTime end, String key, PageParameter page)
    {
    ...
    // Select Count(ID) as ID,Code From dt_User Where CreateTime>'2020-01-24 00:00:00' Group By Code Order By ID Desc limit 20
    static readonly FieldCache<DtUser> _CodeCache = new FieldCache<DtUser>(nameof(Code))
    {
    ...
    // Select Count(ID) as ID,Mail From dt_User Where CreateTime>'2020-01-24 00:00:00' Group By Mail Order By ID Desc limit 20
    static readonly FieldCache<DtUser> _MailCache = new FieldCache<DtUser>(nameof(Mail))
    {
        //Where = _.CreateTime > DateTime.Today.AddDays(-30) & Expression.Empty
    };
    ...
    // Select Count(ID) as ID,Mobile From dt_User Where CreateTime>'2020-01-24 00:00:00' Group By Mobile Order By ID Desc limit 20
    static readonly FieldCache<DtUser> _MobileCache = new FieldCache<DtUser>(nameof(Mobile))
    {
        //Where = _.CreateTime > DateTime.Today.AddDays(-30) & Expression.Empty
    };
